...
本系统是一套专为企业公车报销设计的自动化管理工具。通过图像识别（OCR）与元数据解析技术，自动提取行车仪表盘照片中的**日期、城市地点、经纬度坐标及里程数**，智能生成行程链并填入报销明细表。同时支持**自动解析高速过路费电子票据**，将路桥费自动对齐分配至对应行程，并对照片进行高比例压缩并嵌入 Excel 表格，生成防篡改且包含 GPS 信息的完整行车凭证与报销单。

---

## 核心功能

*   **智能数据及经纬度提取**：使用 `EasyOCR` 提取仪表盘照片中的字面日期、地点及数字；通过专用正则启发式提取并解析水印中的经纬度（纬度/经度，支持对非标准数字如 8 开头纬度进行修正）；支持基于相同地理名称对缺失的经纬度进行智能对齐 and 补全。
*   **行程链自动重构**：自动将所有照片按时间先后和里程递增顺序进行排序，监测相邻照片的位置切换。当检测到位置改变时，自动将行程划分为往返的“行程 Leg”（例如南京 $\rightarrow$ 合肥，合肥 $\rightarrow$ 南京），并记录起止公里数。
*   **智能路桥费自动匹配**：支持自动读取并解析 `trans/trans.pdf` 收费公路通行费电子票据汇总单。系统利用“对称通勤”、“跨省必拆分”和“多段行程并一张”的规律，自动将江苏侧（如曹庄 9.50 元）与安徽侧各段交易路费进行正确组合与去重，并将解析出的单程通行费用智能对齐填入 `用车费用明细.xlsx` 的“过路费”一栏。
*   **Excel 模板智能兼容**：支持在 `用车费用明细.xlsx` 的 `合计` 行上方动态插入多行数据。不仅完整继承原单元格样式（字体、居中、边框），还**完美克服了 openpyxl 插入行时不会自动平移合并单元格的局限**，自动重算和绑定所有汇总公式。
*   **轻量化照片存证 Log**：将所有仪表盘原图（每张约 350KB）按 **15% 等比例缩放压缩**，在报销单下方建立包含“经纬度”与“拍摄日期”双维度的仪表照片存证看板（B列:照片，C列:文件名，D列:总里程，E列:水印地点，F列:经纬度，G列:拍摄日期）。将包含 21 张照片 of Excel 文件体积压缩在 **160 KB** 左右，实现轻量化保存。
...
### 4. 配置文件 (config.json) 说明

为了方便用户灵活配置报销单中的个性化信息及文件路径，系统支持通过根目录下的 `config.json` 文件进行自主配置。

#### 配置项说明

在 `config.json` 中，可配置以下参数：

| 参数名 | 类型 | 说明 | 默认值 |
| :--- | :--- | :--- | :--- |
| `excel_file` | `string` | 最终生成的报销单 Excel 文件路径 | `"用车费用明细.xlsx"` |
| `template_file` | `string` | 报销单基础模板 Excel 文件路径 | `"用车费用明细_template.xlsx"` |
| `img_dir` | `string` | 存放现场仪表盘照片的目录路径 | `"photos"` |
| `title` | `string` | 报销单表格大标题 (写入 Cell A1) | `"2026年4月份出差车辆行车记录表"` |
| `license_plate` | `string` | 报销车牌号 (写入 Cell B2) | `"苏A859EB"` |
| `department_reimburser` | `string` | 报销部门与报销人信息 (写入 Cell C2) | `"报销部门：亥客技术                     报销人：郭良志"` |
| `document_date` | `string` | 制表日期信息 (写入 Cell I2) | `"制表日期："` |

#### 制表日期自动获取机制

系统针对 `document_date` 配置项提供了**智能的日期自动获取与替换机制**：
* **前缀配置**：若您在 `config.json` 中配置为 `"document_date": "制表日期："`，系统在运行时会自动获取当前的系统日期，并拼接为 `"制表日期：YYYY年M月D日"`（例如：`制表日期：2026年5月30日`）填入表格。
* **带日期的配置**：即使配置项中包含旧的日期（例如 `"制表日期：2026年5月28日"`、`"2026-05-28"` 或 `"2026.05.28"`），系统也会自动将其中的日期部分识别并替换为当前的系统日期，保证报销单的制表日期永远是最新的运行时间。
